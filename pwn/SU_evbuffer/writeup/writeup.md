...
inet_pton检测ip不可靠，存在\x00截断

```
inet_pton(AF_INET, temp_recv, &sin->sin_addr)
```

s->user_buf存在溢出，导致s->bev被覆盖
...
最终在add_reference中存在任意函数执行

通过伪造多个bufferevent绕过check，最终实现栈迁移rop
